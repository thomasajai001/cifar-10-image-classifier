...
## API
API is built using Flask framework and hosted on Heroku.

It accepts a POST request with an image.

```python
import requests
import base64

url = 'https://cifa-10-classifier.herokuapp.com/'

with open("<IMG_PATH_HERE>", "rb") as img_file:
    imgdata = img_file.read()

response = requests.post(url, files={"image":imgdata})
